  ...
  return <div className="h-full flex flex-col bg-background border-l">
          ...
          <div className="space-y-3 p-4 my-0 py-0 px-[20px]">
            {regions.map(region => <div key={region.id} className={`p-4 rounded-md border cursor-pointer transition-colors ${selectedRegion?.id === region.id ? 'border-primary bg-primary/5' : 'border-gray-200 hover:bg-gray-50'}`} onClick={() => onRegionSelect(region.id)}>
                <div className="flex justify-between items-start">
                  ...
                  <Button variant="ghost" size="sm" className="h-6 w-6 p-0 opacity-0 group-hover:opacity-100 hover:opacity-100 focus:opacity-100" onClick={e => {
              e.stopPropagation();
      ...
      {selectedRegion && <div className="space-y-4 p-4 py-[6px]">
          ...
          <div className="space-y-2">
            ...
            <Input id="region-name" value={selectedRegion.name} onChange={e => handleChange(e, 'name')} placeholder="Region name" className="h-8" />
          </div>
          
          <div className="space-y-2">
            <Label htmlFor="description" className="text-sm">Description</Label>
            <Textarea id="description" value={selectedRegion.description} onChange={e => handleChange(e, 'description')} placeholder="Optional description" className="resize-none" rows={3} />
          </div>
