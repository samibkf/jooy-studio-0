...
const TextInsert = ({ regions, onRegionUpdate, selectedRegion, onRegionSelect }: TextInsertProps) => {
  ...
  const handleClickAssignedText = (regionId: string) => {
    // Select the region when clicking on its assigned text
                ...
                <div className="space-y-2">
                  {unassignedTexts.map((text, index) => (
                    ...
                    >
                      ...
                      <p className="text-xs line-clamp-2">{text.content.substring(0, 50).replace(/[*\-_]/g, '')}</p>
                    </div>
                  ))}
                </div>
              </ScrollArea>
                ...
                <div className="space-y-2">
                  {assignedTexts.map((text, index) => {
                    ...
                    return (
                      <div
                        key={`assigned-${index}`}
                        onClick={() => assignedRegion && handleClickAssignedText(assignedRegion.id)}
                        className="p-2 border rounded-md border-green-500 bg-green-50 cursor-pointer hover:bg-green-100"
                      >
